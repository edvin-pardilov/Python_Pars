...
def get_product_info(url):
    ...
    data['price'] = soup.find('span', class_='price').next_element.next_element.text#получаем цену
...
try:
    ...
    url = 'http://www.regard.ru/catalog/group4037/page%s.htm'
    #j=0
    for url in [url % i for i in range(1,12)]:# создание цикла для постраничого парса
        #if j % 3 == 0:
            ...
            #print(proxy)
        r = requests.get(url, headers=headers)#Отправляем гет запрос регарду на получение информации с помощью реквеста
        ...
        product_links = (tag.get('href') for tag in soup.select('.header'))#Берем ссылки классов тегов, как бы обошли сам тег а и взли только его ссылки

        for link in product_links:
            print('http://www.regard.ru' + link)
            get_product_info('http://www.regard.ru' + link)#вызываем функцию
            try:
